...
async function seedSaturdayList(
  ...
  const existing = await db
    .prepare(`SELECT COUNT(*) AS count FROM trip_list_items WHERE trip_id = ?`)
    .bind(trip.id)
    .first<{ count: number }>();
  if ((existing?.count ?? 0) > 0) return;

  const histories = new Map(
    RECURRING_PRODUCT_HISTORIES_2026.map((history) => [
      history.itemNumber,
      history,
    ])
  );
  ...
  RECOMMENDATION_SEEDS.forEach((seed, index) => {
    const history = histories.get(seed.itemNumber);
    if (!history) return;
    const lastEvent = history.events.at(-1);
    const reason =
      seed.reasonOverride ??
      `${history.purchaseCount} purchases · usually every ${history.medianIntervalDays} days · last purchased ${history.lastPurchasedOn}`;
    statements.push(
      db
        .prepare(
        ...
        .bind(
          `seed-${trip.scheduled_for}-${seed.itemNumber}`,
          trip.id,
          productIdFor(seed.itemNumber),
          history.canonicalName,
          seed.section,
          reason,
          seed.confidenceBps,
          seed.included ? 1 : 0,
          lastEvent?.unitPriceCents ?? null,
          index,
          now,
          now
        )
